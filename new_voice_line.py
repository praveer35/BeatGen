...
class VoiceLineGenerator:
    def engine(self, data_input):
        ...
        def update_sensibility(arr, index):
            nonlocal sensibility_index
            nonlocal average_entropy
        ...
        def set_repetitions():
            ...
            matchability_noise = 0.1
            switch_measure = 2 if max(matches[0][2], matches[2][0]) >= max(matches[0][1], matches[1][0]) else 1
            ...
            if matches[0][switch_measure] + matchability_noise > matches[switch_measure][switch_measure]:
                #print('measure 0 can repeat during measure 2')
                #print(matches[0][2], matches[2][2])
                forward_switch = True
            if matches[switch_measure][0] + matchability_noise > matches[0][0]:
            ...
            if forward_switch and not backward_switch:
                ...
                measures[switch_measure] = measures[0]
                rhythm[switch_measure] = rhythm[0]
            elif not forward_switch and backward_switch:
                #print('measure 2 subbed into measure 0')
                measures[0] = measures[switch_measure]
                rhythm[0] = rhythm[switch_measure]
            elif forward_switch and backward_switch:
                if matches[0][0] + matches[0][switch_measure] > matches[switch_measure][0] + matches[switch_measure][switch_measure]:
                    #print('measure 0 subbed into measure 2')
                    measures[switch_measure] = measures[0]
                    rhythm[switch_measure] = rhythm[0]
                else:
